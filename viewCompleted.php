...
function display(){
    //using the function from controller to filter only the assignments that I need. In this case completed
    $completedAssessments = getFilterAssessments($_GET['f'], 'Completed');
    if($completedAssessments){
        echo "<form action='viewCompleted.php?f=".$_GET['f']."' method='post'>";
        // foreach to present the assignments with a checkbox for the user mark
        foreach ($completedAssessments as $completedAssessment) {
            //I had to use an arraypop function to not show the word current/completed as the assignment requested
            array_pop($completedAssessment);
            echo implode(" ", $completedAssessment ). "<input type='checkbox' name='$completedAssessment[0]'/>" . "<br>";
        }
...
if(isset($_POST['submit'])){
    ...
    setAssessmentsStatus($assessments, 'Current', $_GET['f']);
}
...
echo show_source("viewCompleted.php")
?>